...
import {util, dia, g, shapes} from '@clientio/rappid/rappid';
import {
  MAX_PORT_COUNT,
  FONT_FAMILY,
  OUT_PORT_HEIGHT,
...
export enum ShapeTypesEnum {
  ...
  FLOWCHART_CANCEL = 'app.FlowchartCancel',
  FLOWCHART_POSTPONE = 'app.FlowchartPostpone',
  LINK = 'app.Link'
}

export const outputPortPosition = (portsArgs: dia.Element.Port[], elBBox: dia.BBox): g.Point[] => {
...
export const outputPortPositionTop = (portsArgs: dia.Element.Port[], elBBox: dia.BBox): g.Point[] => {
  const step = OUT_PORT_WIDTH + PADDING_S;
  return portsArgs.map((port: dia.Element.Port, index: number) => new g.Point({
    x: PADDING_L + OUT_PORT_WIDTH / 2 + index * step,
    y: 0
...
const Base = dia.Element.define(ShapeTypesEnum.BASE, {
  ...
  fromStencilShape: function (element: dia.Element) {
    ...
    const attrs = {
      subject: {
        object_type: element.attr(['subject', 'object_type']),
        subject_type: element.attr(['subject', 'subject_type']),
        role_id: element.attr(['subject', 'role_id']),
        user_id: element.attr(['subject', 'user_id']),
        is_read_only: element.attr(['subject', 'is_read_only']),
        is_subject_changeable: element.attr(['subject', 'is_subject_changeable']),
        is_signature_needed: element.attr(['subject', 'is_signature_needed']),
      },
      label: {
        text: element.attr(['label', 'text']),
        y: element.attr(['label', 'customY'])
      },
      description: {
        text: element.attr(['description', 'text']),
        y: element.attr(['description', 'customY'])
      },
      body: {
    ...
    const ports = {
      ...
      groups: element.attr(['ports', 'groups']),
    }

    return new this({attrs, ports});
  }
...
const Message = Base.define(ShapeTypesEnum.MESSAGE, {
  ...
  ports: {
    ...
    items: [{
      ...
      // , {
      //     group: 'out',
      //     attrs: { portLabel: { text: "Засах" }}
      // }, {
      //     group: 'out',
      //     attrs: { portLabel: { text: "Буцаах" }}
      // }
    ]
  },
  attrs: {
...
const FlowchartCancel = Base.define(ShapeTypesEnum.FLOWCHART_CANCEL, {
  size: {width: 48, height: 48},
  ports: {
    groups: {
      in: {
...
const FlowchartPostpone = Base.define(ShapeTypesEnum.FLOWCHART_POSTPONE, {
  size: {width: 48, height: 48},
  ports: {
    groups: {
      in: {

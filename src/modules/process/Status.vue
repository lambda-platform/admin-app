...
<template>
  <div>
    ...
    <div
      ...
      :class="chooseEmployees ? 'opacity-50 cursor-none pointer-events-none' : 'opacity-100'"
    >
      <div
        v-if="iCanEdit"
      ...
      >
        <a-popover
          ...
          :open="statusHistory[index].open"
          v-if="
            (currentStep.data.subject.object_type !== 'GIVE' &&
              nextStep.step_data.sourcePortLabel !== 'Холбогдох хүмүүст цохсон')
              ? nextStep.step_data.data.subject.object_type === 'END_VOTE'
                ? nextStep.step_data.data.subject.object_type === 'END_VOTE' && props.user_id !== userInfo.id
                : true
              : false
          "
        >
          <template #content>
            <div>
              ...
              <div class="p-2">
                <a-alert
                  v-if="
                    nextStep.step_data.data.description &&
                    currentStep.data.subject.object_type !== 'VOTE' &&
                    nextStep.step_data.data.description
                  "
                  :message="`${getStatusLabel(nextStep.step_data.data.description)}`"
                  type="info"
                  size="small"
                  show-icon
                  style="max-width: 95%"
                ...
                <div
                  v-if="
                    nextStep.status_type !== 'VOTE' &&
                    currentStep.data.subject.object_type !== 'VOTE'
                  "
                >
                  <div>{{ nextStep.step_data.data.label }}</div>
                  <div class="w-full">
                    <div v-if="nextStep.subject_type === 'TO_ROLE'">
                      <RoleUserSelector
                        :disabled="!nextStep.is_subject_changeable"
                        v-model:role_id="nextStep.role_id"
                        v-model:user_id="nextStep.user_id"
                        @change="(d) => changeEmp(nextStep.status_id, d)"
                    ...
                    <div v-if="nextStep.subject_type === 'TO_ORG_STRUCT'">
                      <OrgStructEmployeeSelector
                        :disabled="!nextStep.is_subject_changeable"
                        v-model:org_role_id="nextStep.org_role_id"
                        v-model:org_id="nextStep.org_id"
                        v-model:struct_id="nextStep.struct_id"
                        v-model:job_id="nextStep.job_id"
                        v-model:emp_id="nextStep.emp_id"
                        @change="(d) => changeEmp(nextStep.status_id, d)"
                        mode="TO_ORG_STRUCT"
                      />
                    </div>
              ...
              <signature
                v-if="currentStep.data.subject.is_signature_needed"
                class="mb-2"
                ref="Digitalsignature"
                :action="nextStep.step_data.sourcePortLabel"
                :userSigned="
                  empsignature !== null && empsignature !== undefined && empsignature !== ''
                "
                @signed="Signed"
              ...
              <div class="flex justify-end">
                <span
                  v-if="currentStep.data.subject.object_type !== 'VOTE'"
                  @click="saveAndGoToNext(nextStep, nextStep.step_data.sourcePortLabel)"
                  :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
                    nextStep.status_type
                  )} ${
                    currentStep.data.subject.is_signature_needed && !empsignature
                ...
                >
                  <span>
                    ...
                    <span
                      v-if="
                        nextStep.step_data.data.subject.object_type !== 'END' &&
                        nextStep.step_data.data.subject.object_type !== 'CANCEL'
                      "
                    >
                      шат руу илгээх
                    </span>
                ...
                <span
                  v-else-if="currentStep.data.subject.object_type === 'VOTE'"
                  @click="saveAndGoToNext(nextStep, nextStep.step_data.sourcePortLabel)"
                  :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
                    nextStep.status_type
                  )} ${
                ...
                >
                  <span>
                    {{ nextStep.step_data.sourcePortLabel }} санал илгээх
                  </span>
                </span>
              </div>
            </div>
          ...
          <span
            ...
            :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
              nextStep.step_data.data.subject.object_type
            )}`"
          >
            <span>
              {{
                nextStep.step_data.sourcePortLabel
                  ? getStatusLabel(nextStep.step_data.sourcePortLabel)
                  : getStatusLabel(nextStep.status) + ' шат руу илгээх'
              }}
            </span>
          </span>
        </a-popover>
        <span
          v-if="currentStep.data.subject.object_type === 'GIVE'"
          @click="startTransfer(nextStep, nextStep.step_data.sourcePortLabel)"
          :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
            nextStep.step_data.data.subject.object_type
          )}`"
        >
          <span> Ажилтнуудыг сонгох </span>
        </span>
        <span
          v-if="
            currentStep.data.subject.object_type === 'TRANSFER' &&
            nextStep.step_data.sourcePortLabel === 'Холбогдох хүмүүст цохсон'
          "
          @click="startTransfer(nextStep, nextStep.step_data.sourcePortLabel)"
          :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
        ...
        <span
          v-if="
            nextStep.step_data.data.subject.object_type === 'END_VOTE' &&
            props.emp_id === userInfo.emp_id &&
            nextStep.step_data.sourcePortLabel === 'Зөвшөөрнө'
          "
      ...
      <div class="absolute left-0 flex justify-end pt-2 hidden sm:block">
        <span
          :class="`px-3 py-2 rounded-xl cursor-pointer ${getStatusBgColor(
            currentStep.data.subject.object_type
          )}`"
          @click="save"
          v-if="currentStep.data.subject.object_type !== 'END' && !isVote && iCanEdit"
        >
          {{ t('process.save') }}
        </span>
      </div>
      <div v-if="chooseEmployees">
        <EmployeesSelector
          ...
          :is-transfer="currentStep.data.subject.object_type === 'TRANSFER'"
...
<script setup>
import { defineEmits, defineProps, ref, getCurrentInstance, onMounted, onUnmounted } from 'vue';
import { useI18n } from 'vue-i18n';
import { EventBus } from '~/modules/process/subjectEvent';
import { userInfo, currentStep, iCanEdit } from '~/store/useSiteSettings';
...
const globalSubjectHandler = (payload) => {
  props.statusHistory.forEach((h) => {
    if (
      h.is_done === 0 &&
      h.status_id !== currentStep.value.id &&
      h.step_data
    ) {
      if (payload.emp_id || payload.emp_id === null) h.emp_id = payload.emp_id;
      if (payload.job_id || payload.job_id === null) h.job_id = payload.job_id;
      if (payload.struct_id || payload.struct_id === null) h.struct_id = payload.struct_id;
      if (payload.org_id || payload.org_id === null) h.org_id = payload.org_id;
      if (payload.org_role_id || payload.org_role_id === null) h.org_role_id = payload.org_role_id;
      if (payload.user_id || payload.user_id === null) h.user_id = payload.user_id;
      if (payload.role_id || payload.role_id === null) h.role_id = payload.role_id;

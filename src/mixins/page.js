...
export default {
  ...
  methods: {
    ...
    getPage() {
      this.menu_list = ls.get(MENU_LIST);
      this.menu = ls.get(MENU);
      const permissions = ls.get(PERMISSIONS);
      ...
      if (this.menu_list) {
        let page_index = this.menu_list.findIndex(
          (m) => m.id === this.$route.params.menu_id
        );

        if (page_index === -1) {
          page_index = this.menu_list.findIndex(
            (m) => m.url === this.$route.path
          );
        }
        ...
        if (page_index >= 0) {
          ...
          switch (this.pageType) {
            case "crud":
              ...
              if (crudIndex >= 0) {
                ...
                this.property.form_width = this.cruds[crudIndex].form_width
                ...
                this.property.permissions.gridDeleteConditionJS =
                  this.permissions[page.id].gridDeleteConditionJS;
                this.property.permissions.gridEditConditionJS =
                  this.permissions[page.id].gridEditConditionJS;
                let user_condition = {};
          ...
          if (page.parent?.length >= 1) {
            ...
            const pIndex = page.parent.findIndex(
              (p) => p.link_to === "noActionSubTop"
            );

            // If the parent exists, find its children and set them as the subTopMenus
            if (pIndex >= 0) {
              const { children } = findMenuItemById(
                this.menu,
                page.parent[pIndex].id
              );
